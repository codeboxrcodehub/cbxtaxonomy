...
trait HasSlug
{
    ...
    protected function getSlugSourceString(): string
    {
        ...
        $slugSourceString = collect($this->slugOptions->generateSlugFrom)->map(fn(string $fieldName): string => data_get($this, $fieldName, ''))->implode($this->slugOptions->slugSeparator);
        return $this->generateSubstring($slugSourceString);
    }
    ...
    {
        $query = static::where($this->slugOptions->slugField, $slug)->withoutGlobalScopes();
        if ($this->exists) {
    ...
    {
        return (bool) in_array('CbxTaxonomyScoped\Illuminate\Database\Eloquent\SoftDeletes', class_uses($this));
    }
